...
void Transition::colorize(RgbfColor **dest) {
    ...
    if (G.UhrtypeDef == Ger08x08Viertel && (G.transitionColorize != CHARACTERS)) {
        // set color of A for N, C and H (A has no other word before,
        // compared to N (VIERTEL) and C (NEUN))
        if (dest[1][7].isForeground()) {
            dest[0][7] = dest[1][7];
            dest[2][7] = dest[1][7];
            dest[3][7] = dest[1][7];
        }
        // set color of Z, W for E and I if E and I are active
        // (Z and W is also used for ZWÖLF)
...
uint16_t Transition::transitionCountdown(struct tm &tm) {
    ...
    if (_second != lastSecond) {
        ...
        // not applicable due to linkage to digital clock
